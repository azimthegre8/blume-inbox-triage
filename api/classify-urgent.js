...
module.exports = async (req, res) => {
  ...
  try {
    ...
    for (const threadId of threadsToClassify) {
      try {
        details.push(await fetchThreadDetail(accessToken, threadId));
      } catch (err) {
        console.error("[CLASSIFY_URGENT] Error fetching thread:", err.message);
      }
    }
    ...
    for (const thread of details) {
      try {
        ...
        console.error("[CLASSIFY_URGENT] Classification error:", err.message);
      }
    }
    ...
    res.status(200).json({
      urgent: classified.slice(0, 20).map((item) => ({
        ...
        messageCount: item.email.messageCount,
        urgency: item.classified.urgency,
        urgencyLabel: item.classified.urgencyLabel,
        category: item.classified.category,
        categoryLabel: item.classified.categoryLabel,
        actionRequired: item.classified.actionRequired,
        actionDescription: item.classified.actionDescription,
        deadline: item.classified.deadline,
        recommendedNextStep: item.classified.recommendedNextStep,
        responseTimeframe: item.classified.responseTimeframe,
