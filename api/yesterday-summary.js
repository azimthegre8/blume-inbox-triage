...
async function fetchYesterdayEmails(accessToken) {
  ...
  for (const thread of threads) {
    ...
    emails.push({ threadId: thread.id, from: extractName(from), subject: subject || "(no subject)", snippet: thread.snippet || "", category: categorizeEmail({ from, subject }) });
  }
  return emails;
}
async function handler(req, res) {
  ...
  try {
    ...
    if (!emails.length) {
      return res.status(200).json({ summary: { generated: new Date().toISOString(), totalUnread: 0, categories: {}, message: "No unread emails from yesterday" } });
    }
    const grouped = {};
    ...
    for (const [category, categoryEmails] of Object.entries(grouped)) {
      try {
        ...
        summaries[category] = {
          count: categoryEmails.length, summary: s.categorySummary || "See emails below",
          actionItems: s.keyActionItems || [], pendingResponses: s.pendingResponses || categoryEmails.length,
          criticalDeadlines: s.criticalDeadlines || [], highlights: s.briefHighlights || [],
          emails: categoryEmails.slice(0, 5).map((e) => ({ threadId: e.threadId, from: e.from, subject: e.subject, snippet: e.snippet.substring(0, 100) })),
        };
      } catch (err) {
        summaries[category] = { count: categoryEmails.length, summary: "Manual review required", actionItems: [], pendingResponses: categoryEmails.length, criticalDeadlines: [], highlights: [], emails: categoryEmails.slice(0, 5).map((e) => ({ threadId: e.threadId, from: e.from, subject: e.subject, snippet: e.snippet.substring(0, 100) })) };
      }
    }
    const yd = new Date(); yd.setDate(yd.getDate() - 1);
    res.status(200).json({ summary: { generated: new Date().toISOString(), totalUnread: emails.length, yesterday: yd.toISOString().split("T")[0], categories: summaries } });
  } catch (error) {
